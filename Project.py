...
def draw_creatures():
    ...
    if score > lvl3:
        ...
        choice = random.choice(
            ['t', 't', 't', 't', 'f'])  # choice condition added to randomize/delay when creatures are appearing
        ...
        if len(cre_r) != 0:
            if cre_cyrandom in cre_cy:  # condition to check if a creature already exists in that y-line of the screen
                idx = max(i for i, elem in enumerate(cre_cy) if
                          elem == cre_cyrandom)  # to get the index/position of the most recent creature on the y-line of the screen
                if (cre_cx[idx] >= cre_dist and cre_start[idx] == -50) or (cre_cx[idx] <= cre_dist and cre_start[
                    idx] == 1050):  # condition to check if the creature that already exists, is more than 500 pixels away so that another one can be created
    ...
    if cre_cx != []:
        ...
        for i in range(len(cre_cx)):
            if cre_cx[i] >= -50 and cre_start[
                i] == 1050:  # object moves leftward if it started at right end of the screen and vice versa
